...
if [[ -f "$SCRIPT_DIR/menu.sh" ]]; then
    echo "Installing menu from local files..."
    cp "$SCRIPT_DIR/menu.sh" /usr/local/bin/menu
else
    echo "Downloading menu from Codeberg..."
...
# Install FirewallFalcon SSH config
if [[ -f "$SCRIPT_DIR/ssh" ]]; then
    echo "Applying SSH configuration from local files..."
    cp "$SCRIPT_DIR/ssh" "$SSHD_CONFIG"
else
    echo "Downloading SSH configuration from Codeberg..."
